...
import { Button, Form, Input, Checkbox, Row, Col, Space, Typography, Card, message } from 'antd';
  ...
  const handleFinish = async (values: any) => {
    ...
    try {
      ...
      if (!res.ok) {
         ...
         throw new Error(errorData.error || 'Failed to check save');
      }
    ...
    <div className="max-w-[1200px] mx-auto space-y-6 pb-12">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          />
          <div>
            <Title level={3} style={{ margin: 0, fontWeight: 700 }} className="text-text-primary">Add New Role</Title>
            <Text type="secondary" className="text-sm">Define a new system role and its feature permissions.</Text>
          </div>
        </div>
      </div>
