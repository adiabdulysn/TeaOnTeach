...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = await prisma.ticket_documents.findUnique({
    where: { ticket_document_id: docId },
    select: { file_path: true, original_name: true, ticket_id: true },
  });
